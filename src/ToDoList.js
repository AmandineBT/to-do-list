...
import TaskListSortable from './TaskListSortable.js';
...
export default function ToDoList() {
    ...
    const [dndTasks, setDndTasks] = useState(JSON.parse(localStorage.getItem('dndTasks')) || JSON.parse(localStorage.getItem('tasks'))|| tasksList);
    ...
    return(
      <>
        <div id="actions">
          <SortList onSortListChange={handleSortChange} disabled={notDoneOnly || tasksList.length === 0} allDone = {allDone(tasksList)} zeroDone = {zeroDone(tasksList)} sortChoice={sortChoice} />
          ...
          <br/><label><input type="checkbox" id="check-all" onChange={handleCheckAll} disabled={tasksList.length === 0 }/><span>check/uncheck every task</span></label>
        </div>
        <TaskListSortable dndTasks={dndTasks} setDndTasks={setDndTasks} sortChoice={sortChoice} notDoneOnly={notDoneOnly} tasksList={tasksList} handleChangeClick={handleChangeClick} handleDeleteClick={handleDeleteClick} handleCheckedChange={handleCheckedChange}/>
      </>
      
    );
  }
function SortList({onSortListChange, disabled, sortChoice, allDone, zeroDone}){
  return(
     <>
      <label>Sort
        <select disabled={disabled} name="sort-choices" id="sort-choices" defaultValue={sortChoice} onChange={(e)=> {sortChoiceVar=e.target.value;onSortListChange(sortChoiceVar);}}>
          ...
          <option value='DragAndDrop'>Dragged and dropped</option>
